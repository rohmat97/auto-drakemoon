...
def has_non_black_in_region(dm, x1, y1, x2, y2, direction_name, threshold=0.005):
    '''
...
def strategy_north_west_south_east(dm):
    dm.MoveTo(1022, 352)
    time.sleep(0.67)
    dm.MoveTo(371, 341)
    dm.Delay(50)
...
def strategy_south_east_north_west(dm):
    dm.MoveTo(9, 352)
    time.sleep(0.23)
    dm.Delay(50)
    dm.MoveTo(341, 341)
...
def strategy_north_east_south_west(dm):
    dm.MoveTo(9, 352)
    time.sleep(0.67)
    dm.MoveTo(653, 341)
    dm.Delay(50)
...
def strategy_south_west_north_east(dm):
    dm.MoveTo(1022, 352)
    time.sleep(0.67)
    dm.MoveTo(371, 341)
    dm.Delay(50)
...
STRATEGIES = {
    ...
    ('NorthWest', 'SouthEast'): strategy_north_west_south_east,
...
def execute_battle_strategy(dm, direction, monster_dir):
    ...
    if not strategy:
        bases = ['NorthWest', 'NorthEast', 'SouthEast', 'SouthWest', 'East', 'South', 'West', 'North']
        p_clean = None
        for b in bases:
            if b in direction:
                p_clean = b
        ...
        if p_clean and m_clean:
            strategy = STRATEGIES.get((p_clean, m_clean))
            if not strategy:
                if p_clean == 'NorthWest':
                    strategy = STRATEGIES.get(('NorthWest', 'SouthEast'))
                elif p_clean == 'NorthEast':
                    strategy = STRATEGIES.get(('NorthEast', 'SouthWest'))
                elif p_clean == 'SouthEast':
                    strategy = STRATEGIES.get(('SouthEast', 'NorthWest'))
                elif p_clean == 'SouthWest':
                    strategy = STRATEGIES.get(('SouthWest', 'NorthEast'))
                else:
                    cardinal_mapping = {
                        'NorthWest': 'North',
                        'NorthEast': 'North',
                    ...
                    m_mapped = cardinal_mapping.get(m_clean, m_clean)
                    if p_clean == m_mapped:
                        opposite = {
                            'East': 'West', 'West': 'East',
                            'North': 'South', 'South': 'North'
                        }
                        m_mapped = opposite.get(p_clean)
                    strategy = STRATEGIES.get((p_clean, m_mapped))
            
            if strategy:
                print(f"Mapped strategy match: ({direction}, {monster_dir}) -> ({p_clean}, {strategy.__name__ if hasattr(strategy, '__name__') else str(strategy)})")
